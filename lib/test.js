...
function Test(app, method, path, domain) {
  ...
  this.url = typeof app === 'string'
    ? app + path
    : this.serverAddress(app, path, domain);
}
...
Test.prototype.serverAddress = function(app, path, domain) {
  ...
  return protocol + '://' + (domain || '127.0.0.1') + ':' + port + path;
};
